	...
	{   
		...
		if(dur<50000&&dur%5000==0)
		{
		  LED0=!LED0;
			...
			if((index_temp>=TIME_INTERVAL-1)&((index_temp+1)%TIME_INCREMENT==0)&(kill_flag==1))//kill_flag的作用是只执行一次if后的语句
        ...
        switch ( data_cnt_state )
		{
			...
			case 2:
			    ...
			    if (data_cnt_count%2 ==0)
					data_buff[data_cnt_count %(DATA_CHANNEL*2)/2]= res*256;
				else
				{
			      ...
			      if(data_buff[data_cnt_count%(DATA_CHANNEL*2)/2]>=65534/2)   
				        ...
				        data_buff[data_cnt_count %(DATA_CHANNEL*2)/2]-=1862;
				        data_buff[data_cnt_count %(DATA_CHANNEL*2)/2]=Filter(data_buff[data_cnt_count %(DATA_CHANNEL*2)/2]);
				}
				data_cnt_count++;
