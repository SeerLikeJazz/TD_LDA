...
//LZX//
 //接收数组
u8 Received_buff[12]={0};
u8 Receive_complete_Flag=0;
/////////////
u8					filter_n;
...
void USART3_IRQHandler(void)
{
	...
	{	 
        ...
        switch ( data_cnt_state )
		{
			...
			case 2:
				data_cnt_check+=res;
			  Received_buff[data_cnt_count] = res;

			
//			  if (data_cnt_count%3 == 0)
//				{
//			  data_buff[data_cnt_count % data_cnt_max/3*2]= res*16;//LZX//<移位更高效,过会儿优化一下>
//				}
//				else if(data_cnt_count%3 == 1)
//				{				
//				data_buff[data_cnt_count % data_cnt_max/3*2] += (res>>4);
//				data_buff[data_cnt_count % data_cnt_max/3*2] = Filter(data_buff[data_cnt_count % data_cnt_max/3*2]);
//				data_buff[data_cnt_count % data_cnt_max/3*2 + 1] = ((res&0x0F)*256);
//				}
//				else
//				{
//				data_buff[data_cnt_count % data_cnt_max/3*2 + 1] += (res);
//				data_buff[data_cnt_count % data_cnt_max/3*2 + 1] = Filter(data_buff[data_cnt_count % data_cnt_max/3*2 + 1]);				
//				}
...
//收到的12字节数据存储为8个浮点数据，之后SetData(data_buff);
void Data_12_8_transform(void)
{
	...
	for(i=0;i<data_cnt_max;i++)
	{
			  if (i%3 == 0)
				{
			  data_buff[i % data_cnt_max/3*2]= Received_buff[i]*16;//LZX//<移位更高效,过会儿优化一下>
				}
				else if(i%3 == 1)
				{				
				data_buff[i % data_cnt_max/3*2] += (Received_buff[i]>>4);
				data_buff[i % data_cnt_max/3*2] = Filter(data_buff[i % data_cnt_max/3*2]);
				data_buff[i % data_cnt_max/3*2 + 1] = ((Received_buff[i]&0x0F)*256);
				}
				else
				{
				data_buff[i % data_cnt_max/3*2 + 1] += (Received_buff[i]);
